"""
Gradio UI for the Simulation Logic module.

Two simulation modes:
  1. Full Simulation — runs all days at once, shows complete logs
  2. Day-by-Day — click "Next Day" to step through one day at a time

Runs on port 7861 (separate from the main server on 7860).

Usage:
    python -m simulation_logic.app
"""
...
import gradio as gr
import pandas as pd
from simulation_logic.simulation import run_simulation, init_simulation, step_one_day
from simulation_logic.models import SimulationResult, DayLog
...
_day_state = None          # SimState object
    ...
    for day_log in logs:
        ...
        if day_log.actions:
            ...
            for action in day_log.actions:
                ...
                lines.append(f"- {icon} **{action.type.upper()}**{target}{amount_str}: {action.reasoning}")
            lines.append("")
...
def _build_day_chart():
    """Build chart data from accumulated day logs."""
    if not _day_logs:
        return _empty_chart()
    return pd.DataFrame({
...
def _format_result(result: SimulationResult):
    """Convert SimulationResult into UI-friendly outputs."""
...
def build_ui():
    """Build the Gradio interface with two tabs: Full and Day-by-Day."""
    with gr.Blocks(title="Cashflow Simulation") as demo:
        ...
        with gr.Row():
            ...
            sim_window = gr.Slider(
                minimum=3, maximum=14, value=7, step=1,
